...
export default function VehiclesPage() {
  ...
  const columns = useMemo<ColumnDef<Vehicle>[]>(() => [
    ...
    { id: 'actions', header: 'Actions', cell: ({ row }) => canMutate ? <div className="flex gap-1"><button className="btn-secondary !h-9 !px-2" title="Modifier" onClick={() => begin(row.original)}><Pencil size={16}/></button><button className="btn-secondary !h-9 !px-2 text-red-600" title="Supprimer" onClick={() => remove(row.original.id)}><Trash2 size={16}/></button></div> : null },
  ], []);
  const set = (key: keyof typeof empty, value: string) => setForm((f) => ({ ...f, [key]: value }));
  return <><PageHeader title="Vehicules" description="Parc automobile couvert par les contrats" action={<div className="flex gap-2">{canMutate && <><input ref={fileRef} type="file" accept=".xlsx" className="hidden" onChange={(e) => void importFile(e.target.files?.[0])}/><button className="btn-secondary" onClick={() => fileRef.current?.click()}><Upload size={18}/>Importer</button></>}<button className="btn-secondary" onClick={download}><Download size={18}/>Excel</button>{canMutate && <button className="btn-primary" onClick={() => begin()}><Plus size={18}/>Ajouter</button>}</div>} /><DataTable {...list} columns={columns} onSearch={list.setSearch} onPage={list.setPage}/>
