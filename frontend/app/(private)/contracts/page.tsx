...
export default function ContractsPage() {
  ...
  return <><PageHeader title="Contrats" description="Suivi des polices flotte et de leurs echeances" action={<button className="btn-primary" onClick={() => begin()}><Plus size={18}/>Ajouter</button>} /><DataTable {...list} columns={columns} onSearch={list.setSearch} onPage={list.setPage} />
    <Modal title={renewing ? `Renouveler ${renewing.number}` : editing ? 'Modifier le contrat' : 'Nouveau contrat'} open={open} onClose={() => setOpen(false)}><form onSubmit={save} className="grid gap-4 sm:grid-cols-2">
      <label><span className="label">Numero</span><input required className="field" value={form.number} disabled={!!editing} onChange={(e) => set('number', e.target.value)} /></label>
      ...
      <label><span className="label">Date de debut</span><input required type="date" className="field" value={form.startDate} onChange={(e) => set('startDate', e.target.value)} /></label>
      <label><span className="label">Date de fin</span><input required type="date" className="field" value={form.endDate} onChange={(e) => set('endDate', e.target.value)} /></label>
